...
function last($userId, $bike = 0)
{
    ...
    $bikeNum=intval($bike);
    if ($bikeNum) {
        $result=R::getAll("SELECT userName,parameter,standName,action,time FROM `history` JOIN users ON history.userid=users.userid LEFT JOIN stands ON stands.standid=history.parameter WHERE bikenum=$bikeNum AND (action NOT LIKE '%CREDIT%') ORDER BY time DESC LIMIT 10");
        ...
        while ($row=$result->fetch_assoc()) {
            ...
            if ($row["standName"]!=null) {
                ...
                $result2=R::getAll("SELECT time FROM history WHERE bikeNum=".$row["bikeNum"]." AND userId=".$row["userId"]." AND action='RENT' ORDER BY time DESC");
                $row2=$result2->fetch_assoc();
                $historyInfo.=": ".date("d/m H:i", strtotime($row2["time"])).'</span>';
            }
                $result2=R::getAll("SELECT note FROM notes WHERE bikeNum='".$row["bikeNum"]."' AND deleted IS NULL ORDER BY time DESC");
                $note="";
...
function revert($userId, $bikeNum)
{
    ...
    $standId=0;
    $result=R::getAll("SELECT currentUser FROM bikes WHERE bikeNum=$bikeNum AND currentUser IS NOT NULL");
    if (!$result->num_rows) {
...
function register($number, $code, $checkcode, $fullname, $email, $password, $password2, $existing)
{
    ...
    if (issmssystemenabled()==true) {
        $result=R::getAll("SELECT parameter FROM history WHERE userId=0 AND bikeNum=0 AND action='REGISTER' AND parameter='$parametercheck' ORDER BY time DESC LIMIT 1");
        if ($result->num_rows==1) {
            if (!$existing) { // new user registration
                $result=R::exec("INSERT INTO users SET userName='$fullname',password=SHA2('$password',512),mail='$email',number='$number',privileges=0");
            ...
            {
                $result=R::getAll("SELECT userId FROM users WHERE number='$number'");
                $row=$result->fetch_assoc();
                $userId=$row["userId"];
    ...
    } else // SMS system disabled
      {
        ...
        $userId=$db->conn->insert_id;
        $result=R::exec("UPDATE users SET number='$userId' WHERE userId='$userId'");
        sendConfirmationEmail($email);
...
function smscode($number)
{
    ...
    if (!issmssystemenabled()) {
        $result=R::exec("INSERT INTO sent SET number='$number',text='$text'");
    }
    $result=R::exec("INSERT INTO history SET userId=0,bikeNum=0,action='REGISTER',parameter='$number;$smscodenormalized;$checkcode'");
...
function trips($userId, $bike = 0)
{
    ...
    $bikeNum=intval($bike);
    if ($bikeNum) {
        $result=R::getAll("SELECT longitude,latitude FROM `history` LEFT JOIN stands ON stands.standid=history.parameter WHERE bikenum=$bikeNum AND action='RETURN' ORDER BY time DESC");
        while ($row = $result->fetch_assoc()) {
