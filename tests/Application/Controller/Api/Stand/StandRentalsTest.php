    ...
    public function testListsRentalsOriginatingAtTheStand(): void
    {
        ...
        $db->query(
            "INSERT INTO history (userId, bikeNum, action, parameter, time)
             VALUES (:u, :b, 'RETURN', :stand, '2026-05-01 10:00:00')",
            ['u' => $userId, 'b' => self::SENTINEL_BIKE, 'stand' => (string)self::STAND_ID]
        );
        $db->query(
            "INSERT INTO history (userId, bikeNum, action, parameter, time)
             VALUES (:u, :b, 'RENT', 'TESTCODE', '2026-05-01 11:00:00')",
            ['u' => $userId, 'b' => self::SENTINEL_BIKE]
        );
